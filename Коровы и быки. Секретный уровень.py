...
def check(bot_num_: int) -> bool:
    global dict_
    l1 = number_to_list(bot_num_)
    l1 = [l1[q] for q in range(len(l1)) if q not in dict_['bulls'].keys()]
...
while bot_num != human_num:
    ...
    for i in range(len(bot_num)):
        if i not in dict_['bulls'].keys():
            bot_num[i] = random.choice([str(j) for j in range(10) if str(j) != bot_num[i]])
